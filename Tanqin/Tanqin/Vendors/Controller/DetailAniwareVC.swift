...
enum DetailAniwareLayout {
    case info, describe, season, episodes, suggest, more_info ,ads
}

class DetailAniwareVC: BaseController {
    ...
    fileprivate let collectionView: UICollectionView = {
        ...
        view.registerItem(cell: DetailAniwareInfoCell.self)
        view.registerItem(cell: DetailAniwareMoreInfoCell.self)
        view.registerItem(cell: DetailAniwareDescribeCell.self)
        view.registerItem(cell: DetailAniwareSeasonCell.self)
        view.registerItem(cell: DetailAniwareEpisodesCell.self)
        view.registerItem(cell: DetailAniwareSuggestCell.self)
    ...
    fileprivate let layouts: [DetailAniwareLayout] = [.info, .ads, .describe, .season, .more_info, .episodes, .suggest]
    fileprivate let layoutsNotMoreInfo: [DetailAniwareLayout] = [.info, .ads, .describe, .season, .episodes, .suggest]
    ...
    override func viewDidLoad() {
        ...
        AnWHTMLService.shared.detail(link: aniModel?.detailLink ?? "") { [weak self] detail in
...
extension DetailAniwareVC: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case .info:
            let cell: DetailAniwareInfoCell = collectionView.dequeueReusableCell(indexPath: indexPath)
            ...
            return cell
        case .more_info:
            let cell: DetailAniwareMoreInfoCell = collectionView.dequeueReusableCell(indexPath: indexPath)
            cell.data = self.aniDetailModel
            ...
            return cell
        case .describe:
            let cell: DetailAniwareDescribeCell = collectionView.dequeueReusableCell(indexPath: indexPath)
            cell.data = self.aniDetailModel
            cell.numberLineDes = self.numberLineDes
            cell.onSelectedReadmore = {
                if self.numberLineDes == 0 {
            ...
            return cell
        case .season:
            let cell: DetailAniwareSeasonCell = collectionView.dequeueReusableCell(indexPath: indexPath)
            cell.data = self.aniDetailModel?.season ?? []
            cell.linkSelect = self.aniModel?.detailLink ?? ""
            cell.onSelected = { season in
                self.openSeason(season: season)
            }
            return cell
        case .episodes:
            let cell: DetailAniwareEpisodesCell = collectionView.dequeueReusableCell(indexPath: indexPath)
            ...
            return cell
        case .suggest:
            let cell: DetailAniwareSuggestCell = collectionView.dequeueReusableCell(indexPath: indexPath)
    ...
    func openSeason(season: AniSeasonModel) {
        if self.aniModel?.detailLink == season.detailLink {
            return
        }
        
        AnWHTMLService.shared.detail(link: season.detailLink) { [weak self] detail in
            guard let self = self else { return }
            
            DispatchQueue.main.async {
                self.aniDetailModel = detail
                self.aniModel = detail?.toAniModel(link: season.detailLink)
                self.collectionView.reloadData()
            }
        }
    }
    ...
    func watchAnime(episode: AniEpisodesModel) {
        ...
        recentAni.episode = episode.episode
        recentAni.sourceId = episode.sourceID
        RecentAni.addRecentAni(recentAni: recentAni)
        loadingView.show()
        
        ServService.shared.fetchServer(link: link, name: name, season: episode.season, episode: episode.episode, sourceId: episode.sourceID) { [weak self] data in
...
extension DetailAniwareVC: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        switch layout[indexPath.row] {
        case .info:
            return DetailAniwareInfoCell.size(width: width)
        case .describe:
        ...
        case .more_info:
            if numberLineDes == 0 {
                return .init(width: width, height: 184)
            } else {
                return .init(width: width, height: 0)
            }
        case .season:
            if (self.aniDetailModel?.season ?? []).count == 0 {
                return .init(width: width, height: 0)
            } else {
                return DetailAniwareSeasonCell.size(width: width)
            }
        case .episodes:
            var height = 0
        ...
        case .suggest:
            if (self.aniDetailModel?.suggestion ?? []).count == 0 {
                ...
                return DetailAniwareSuggestCell.size(width: width)
            }
